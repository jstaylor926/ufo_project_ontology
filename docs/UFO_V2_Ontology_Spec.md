...
| 6 | `index.ts` (priority driver) | `priorityDriver`, `priorityDriverTester`, `calculateOneEntryScore`, `sbcBumpAndPriorityCalc`, `setMainConfig` (pure: `calculateGlobalScore`, `scoreOne`, `getParameterValues`, `spanOrBlock`, `daysSince`, `normalizeDateSpan`) | UfoEntry, PriorityAlgorithm | `src/prioritization/{types,bounds,parameters,score,algorithm,adapter}.ts` | ☑ Ported (pure layer + adapter, vitest 98/98) — pending smoke deploy and FBP bind. `setSharedProperties`/`updatelatestRts` deferred to port #7 (FSR entry driver) where they actually live in V1. |
...
| — | `v2compat.ts` | `V2_TO_V1_PRIORITY_KEYS`, `toV1PriorityKey` | (transitional bridge) | n/a — retires at end of P4 | ☐ Deprecated (2026-05-14, banner added). Physical removal pending V1 priority binding decommission in Ontology Manager. V2 functions consume V2 parameter keys natively via `src/prioritization/parameters.ts`. |
